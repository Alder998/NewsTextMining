...
cleanData = ns.PreProcessing(textList, target='returns', classes=4, threshold=1.2, databaseVersion='V2').preProcess(POS_tagging=False)

# Data Vectorization: turning text data into a vector, numerically processable by an algorithm
BoWEmbedding = ns.Vectorize(cleanData).Embedding(method = 'Bag-of-Word', vectorSize=20, components = 200)
...
modelSet = ns.NNModel(sample, epochs=10).NNProcessing(NNType='FF', shapeRec=[64], shape = [200, 200],
                                                                        activation = 'relu')
